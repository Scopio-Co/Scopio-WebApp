...
class SocialAdapter(DefaultSocialAccountAdapter):
    def is_auto_signup_allowed(self, request, sociallogin):
        ...
        logger.info(f"is_auto_signup_allowed called for {sociallogin.account.provider}")
        return True
    ...
    def pre_social_login(self, request, sociallogin):
        ...
        try:
            ...
            if sociallogin.is_existing:
                logger.info(f"Social login already exists for {sociallogin.user}")
                return
            ...
            try:
                ...
                logger.info(f"Deleted duplicate users: {duplicate_ids}")

            # Check if the existing user has a password (form signup) or not (OAuth only)
            has_password = existing_user.has_usable_password()
            if has_password:
                logger.info(f"User {existing_user.email} has a form-based account, linking Google account")
            else:
                logger.info(f"User {existing_user.email} is OAuth-only, linking to social account")
            
            # Link this social login to the existing user; login will proceed
            # without showing the 3rdparty signup page
            sociallogin.connect(request, existing_user)
            logger.info(f"Connected social login to existing user: {existing_user.email}")
