...
class CookieTokenObtainPairView(TokenObtainPairView):
    def post(self, request, *args, **kwargs):
        ...
        if username:
            ...
            try:
                ...
                if user and not user.has_usable_password():
                    ...
                    if has_google:
                        return Response(
                            {"detail": "This account was created with Google. Please sign in using 'Continue with Google' instead."},
        ...
        try:
            ...
            response = Response(
                {"access": access, "refresh": refresh, "detail": "login successful"},
...
class CookieTokenRefreshView(TokenRefreshView):
    def post(self, request, *args, **kwargs):
        ...
        access = resp.data.get("access")
        refresh = resp.data.get("refresh")
        response = JsonResponse({"detail": "refresh successful", "access": access, "refresh": refresh})
        _set_auth_cookies(response, access, refresh)
        return response
...
def auth_profile(request):
    ...
    serializer = ProfileSettingsSerializer(data=request.data, partial=True, context={'request': request})
    serializer.is_valid(raise_exception=True)
...
def users_list(request):
    ...
    return Response({"total_users": users.count(), "users": list(users)}, status=status.HTTP_200_OK)
